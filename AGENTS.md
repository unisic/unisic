...
- **X11 sessions ARE supported, as a deliberate second target (since 0.8) - not as a hack.** Everything session-agnostic (screenshots through the portal, overlay, editor, OCR, history, upload) always worked there; two paths were added for `xcb` sessions only:
  ...
  - Both are compiled unconditionally in the kit (`HAVE_X11` and `HAVE_X11_HOTKEYS` are hard build requirements - see §3) and runtime-gated on `QGuiApplication::platformName() == "xcb"`. **Additive only:** an X11 branch must never change what a Wayland session does. `CaptureManager::isX11Session()` exists for the reverse case (a stale `WAYLAND_DISPLAY` must not route an X11 session's capture into a dead or foreign compositor).
...
- **Required - and that is the ONLY kind there is. Unisic has no optional dependencies.** Every library below is a hard build requirement: a missing one stops `cmake -B build` at CONFIGURE time with a `FATAL_ERROR` naming the package on four distros (`unisic_require_failed()` in `CMakeLists.txt`, `unisic_kit_require_failed()` in the kit). Nothing compiles out and nothing degrades into "the feature is simply absent" - that was tried for four releases and it produced binaries whose UI promised what the build had silently dropped (`HAVE_LIBINPUT` off in every Flatpak, `HAVE_KGUIADDONS` off in the released rpm, `HAVE_X11` off in the Nix build, every job green). The eleven `HAVE_*` defines survive as the NAMES of their code paths, not as switches: a successful configure writes `<build>/unisic-features.txt` listing all ten distinct ones as `NAME=ON` (the kit contributes four, `HAVE_PIPEWIRE` is shared), and any build that cannot produce that file is not a build we ship.
  - Qt 6.5+, all of it: `Core Gui Widgets Quick Qml QuickControls2 DBus Network Concurrent Svg`, plus `WaylandClient`, the Qt6 GUI **private** headers and `LinguistTools` → `qt6-qtbase-devel qt6-qtdeclarative-devel qt6-qtsvg-devel qt6-qtwayland-devel qt6-qtbase-private-devel qt6-qttools-devel` (Debian: `qt6-base-dev qt6-declarative-dev qt6-svg-dev qt6-wayland-dev qt6-base-private-dev qt6-tools-dev qt6-l10n-tools`; Arch: `qt6-base qt6-declarative qt6-svg qt6-wayland qt6-tools`). `LinguistTools` → `HAVE_TRANSLATIONS` bakes the seven `.qm` catalogs into the qrc; without it the binary would ship English only while the language picker still offers seven, which is why it is not negotiable either.
  - `pipewire-devel` (pkg-config `libpipewire-0.3`) → `HAVE_PIPEWIRE` → GIF/screen recording. Probed on BOTH sides (the app links it, the kit builds the grabber), so no build can exist that cannot record.
  - `tesseract-devel leptonica-devel` (pkg-config `tesseract lept`) → `HAVE_TESSERACT` → OCR ("copy text from capture"). `src/ocr/OcrEngine.*` is always compiled and `App.ocrAvailable` is always true. The language DATA is a separate, equally hard requirement of the PACKAGE rather than of the build: every recipe ships `eng` + `pol` + `osd` (`tesseract-langpack-eng` and friends), because a build with the engine and no traineddata OCRs nothing.
  - `zxing-cpp-devel` (`find_package(ZXing CONFIG)`) → `HAVE_ZXING` → QR/barcode payload instead of OCR-ing a code's pixels. It still sits inside the OCR path, but that path is required too, so the nesting can no longer turn it off.
  - `layer-shell-qt-devel wayland-devel` → `HAVE_LAYERSHELL` → the styled capture card. On Plasma this is the ONLY route to it (the XWayland helper is refused while `org.kde.KWin` is on the bus), which is exactly why it is mandatory: every build has the card on the most capable desktop there is.
  - `libinput-devel systemd-devel` → `HAVE_LIBINPUT` → click capture AND the pressed-key badge. Needs BOTH pkg-config modules (`libinput`, `libudev`); on Fedora `libudev.pc` ships in `systemd-devel`. `InputPermission::probe()` can therefore only ever answer about the user's own device access, never about the build. (History, and the argument for the whole policy: no recipe carried libinput until 0.8.3, so every package before that shipped the feature compiled out while the Settings rows blamed "this build" - see `docs/dev/packaging.md` for the full list of files to change together.)
  - `kf6-kguiaddons-devel` → `HAVE_KGUIADDONS` → `KSystemClipboard`, i.e. screenshots entering Plasma's Klipper clipboard **history**.
  - `qt6-qtwayland-devel qt6-qtbase-private-devel plasma-wayland-protocols-devel` (all three) → `HAVE_KWIN_SCREENCAST` in unisic-kit → KWin-native recording with no portal share dialog. Debian 13 ships the private headers but no `Qt6GuiPrivateConfig.cmake` at all, so the kit falls back to `find_path()` for `qpa/qplatformnativeinterface.h` and prints which of the two routes it took - do not "simplify" that fallback away, it is the only thing keeping trixie buildable.
  - `libX11-devel libXext-devel libXfixes-devel` (pkg-config `x11 xext xfixes`) → `HAVE_X11` in unisic-kit → `X11ShmGrabber`, the frame source on an X11 session. **The app-side use is `#if defined(HAVE_PIPEWIRE) && defined(HAVE_X11)`** - the sampler/encoder it feeds is compiled under `HAVE_PIPEWIRE` - and since both are hard requirements that combination is always satisfied.
  - `libX11-devel libxcb-devel` (pkg-config `x11 xcb`) → `HAVE_X11_HOTKEYS` in unisic-kit → `X11Hotkeys` (`XGrabKey` + the xcb `KeyPress` native event filter). Still a **separate** probe from `HAVE_X11` because it names different code and wants different packages, so the error message can be specific; neither can be off, so the old split builds (hotkeys without recording, or the reverse) no longer exist.
  - Packaging carries all of the above as RUNTIME dependencies too: `libx11 libxext libxfixes libxcb` are in `packaging/arch/PKGBUILD` (both `depends` and `makedepends`) and the equivalent lists elsewhere. A missing X11 lib there ships a binary that will not start - that is what stalled the 0.8 AUR publish.
- **After a dependency appears in an EXISTING build tree that configured without it, delete `build/`.** Re-running `cmake -B build` adds the new sources to the target but the AUTOMOC custom command does not depend on `AutogenInfo.json`, so ninja never re-runs it: `mocs_compilation.cpp` keeps the old list and the link dies with `undefined reference to vtable for <NewClass>` / `staticMetaObject` / its signals. Deleting just the stale `<build>/<target>_autogen/timestamp` (e.g. `build/external/unisic-kit/unisic-kit_autogen/timestamp`) forces AUTOMOC to re-run and is enough if a full rebuild is too expensive. A clean configure has never had the problem.
- **Runtime helpers shelled out, not linked:** `ffmpeg` (GIF/video encode), `curl` (FTP/SFTP uploads), `grim` (wlroots/niri capture), `wl-copy` (clipboard mirror), `kbuildsycoca6` (KDE service-cache rebuild), `pw-dump` (audio-source listing). These are PROGRAMS found at runtime with `QStandardPaths::findExecutable`, not build gates - the no-optional-dependencies rule is about compile-time gates, and a helper the user removed from their own machine must still be detected and degraded around, never crash. Every package declares all feature helpers (`ffmpeg`, `curl`, `grim`, `zip`, `wl-clipboard`, `pw-record`/`pw-dump`/`pw-play`) and one desktop-specific portal backend as hard runtime dependencies, or bundles the helpers in Flatpak/AppImage while the sandbox/session supplies its portal. "Absent" is a hand-broken system, not a shipped state. Split-package landmine: Debian's three PipeWire CLIs are `pipewire-bin`; Fedora's are `pipewire-utils`; Arch's `pw-dump` is in `pipewire` while `pw-record`/`pw-play` are in `pipewire-audio`.
...
- **A new dependency is mandatory from the start or it does not land.** There is no third option: the `HAVE_*` pattern names a code path, it is not permission to ship a build without it. Adding a gate means adding a `unisic_require_failed()` branch AND its packages to every recipe in the same commit (`docs/dev/packaging.md` lists them), and a dependency that some channel cannot carry - Flatpak included, where whatever the manifest does not build the sandbox can never borrow from the host - is a dependency we do not take.
...
src/
  ...
  ocr/                 OcrEngine (HAVE_TESSERACT, i.e. always built).
...
scripts/               install.sh - END-USER universal installer/updater (NOT the build; see §3).
                       Nothing installed yet → the self-updating AppImage (no password); the
                       distro package (deb/rpm[Fedora]/pkg.tar.zst, OBS zypper repo) is the
                       second menu entry. An existing install is updated on its own channel.
                       ...
                       Confirms to X11 users that recording and hotkeys work there.
                       Also: build-appimage.sh, gen-changelog.sh, vm-test.sh.
```
...
- [ ] **No new dependency** (Qt module, `.so`, bundled lib) without justification + sign-off. Any dependency that does land is REQUIRED - a `unisic_require_failed()` branch at configure time, and its packages added to `packaging/**`, `.github/workflows/**`, `unisic.spec`, `nix/package.nix` and the Flatpak manifest in the SAME commit.
