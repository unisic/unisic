...
- **Translate every user-facing string.** Wrap it in `qsTr()` (QML) / `tr()` (C++) **and** fill it in all five `i18n/unisic_{en,pl,es,it,fr}.ts` files (English == source text). Workflow: add the call → `cmake --build build --target update_translations` (lupdate appends new strings as `type="unfinished"`) → fill every unfinished `<translation>` in all five and drop the marker. A plain build bakes `.qm` into the qrc, so `unfinished`/empty entries silently fall back to English - never ship those. Keep placeholders (`%1`), tokens (`%date%`), globs, and tool names verbatim; match the existing per-language sound-cue convention. New QML files must be listed in `qt_add_qml_module` so lupdate scans them.
