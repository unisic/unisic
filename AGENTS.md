...
- **Runtime helpers shelled out, not linked:** `ffmpeg` (GIF/video encode), `curl` (FTP/SFTP uploads), `grim` (wlroots/niri capture), `wl-copy` (clipboard mirror), `kbuildsycoca6` (KDE service-cache rebuild), `pw-dump` (audio-source listing). These are PROGRAMS found at runtime with `QStandardPaths::findExecutable`, not build gates - the no-optional-dependencies rule is about compile-time gates, and a helper the user removed from their own machine must still be detected and degraded around, never crash. Every package still declares the ones a feature needs (`ffmpeg`, `curl`, `wl-clipboard`) as hard runtime dependencies, so "absent" is a hand-broken system, not a shipped state.
