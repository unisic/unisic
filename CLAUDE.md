...
**Every dependency is required. Unisic has none of the optional kind, and no feature that compiles out.** Fedora: `qt6-qtbase-devel qt6-qtdeclarative-devel qt6-qtsvg-devel qt6-qtwayland-devel qt6-qtbase-private-devel qt6-qttools-devel plasma-wayland-protocols-devel pipewire-devel tesseract-devel leptonica-devel tesseract-langpack-eng zxing-cpp-devel layer-shell-qt-devel wayland-devel kf6-kguiaddons-devel libinput-devel systemd-devel libX11-devel libXext-devel libXfixes-devel libxcb-devel`, plus runtime `ffmpeg`, `curl`, `grim`, `zip`, PipeWire tools and `wl-clipboard`. Miss one and `cmake -B build` FAILS at configure time naming the package on four distros - that is the whole point, because a build that quietly drops a feature ships that hole to everyone who installs that package, and inside a Flatpak sandbox the user cannot even add the library afterwards. The eleven `HAVE_*` defines survive as the names of the code paths (`HAVE_PIPEWIRE`, `HAVE_TESSERACT`, `HAVE_ZXING`, `HAVE_LIBINPUT`, `HAVE_LAYERSHELL`, `HAVE_KGUIADDONS`, `HAVE_TRANSLATIONS` in the app; `HAVE_PIPEWIRE`, `HAVE_X11`, `HAVE_X11_HOTKEYS`, `HAVE_KWIN_SCREENCAST` in the kit) and every one of them is ON in every build and every packaging recipe - a successful configure writes `<build>/unisic-features.txt` saying exactly that. The capture popup positions itself by filling the screen and masking input to the card (no layer-shell dependency). `ctest --test-dir build` runs the QtTest targets in `tests/` (pure-logic units - version compare, shortcut format, annotation canvas, history filter); everything compositor-bound is covered by the in-app smoke test instead.
...
- **Every count or list a change invalidates is fixed in the same commit**: `README.md` enumerates shipped languages, editor tool count, themes, formats, sessions and default hotkeys, and nothing builds or tests those numbers, so a wrong one survives until it misleads a user. Grep the same fact in `CLAUDE.md`, `AGENTS.md`, `docs/dev/**` and the sibling `unisic-website` repo (`content/docs/**`, `lib/i18n/dictionaries/**`) - update it there too, or state plainly that it still needs updating if that repo is not available. Do not wait to be asked (`AGENTS.md` §9).
