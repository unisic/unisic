...
- **OCR detects the language for you — now across scripts**: on by default, it detects the script of each capture (Latin, Arabic, Hebrew, Chinese/Japanese/Korean, Devanagari, and more) and recognizes with just the matching installed language packs — faster and more accurate than loading them all. Needs the Tesseract “osd” data pack; without it, it falls back to loading every installed pack. Install the packs for the scripts you use; turn it off in Settings › OCR to pin a specific set.
...
- **OCR sam wykrywa język — teraz na różnych pismach**: domyślnie włączony, wykrywa pismo każdego zrzutu (łacińskie, arabskie, hebrajskie, chińskie/japońskie/koreańskie, dewanagari i inne) i rozpoznaje tylko pasującymi zainstalowanymi pakietami — szybciej i dokładniej niż ładowanie wszystkich. Wymaga pakietu danych „osd” Tesseract; bez niego wraca do ładowania wszystkich zainstalowanych paczek. Zainstaluj paczki dla pism, których używasz; wyłącz w Ustawienia › OCR, aby ustawić konkretny zestaw.
