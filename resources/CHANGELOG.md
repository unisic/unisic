...
### English
**Improved**
- **A short tour of the things that are easy to miss**: six cards covering the letter each annotation tool answers to, the keys the selection overlay and the editor use while they own the keyboard, opening a file by dropping or pasting it, retaking the shot you just took, custom upload servers, and instant replay. It is never forced on you: it is offered once after the welcome screen and otherwise sits in Settings › General, it changes nothing on its own, and you can leave it at any card. The tool letters it shows are read from the toolbar itself, so they cannot go stale.
...
### Polski
**Ulepszone**
- **Krótkie oprowadzanie po rzeczach, które łatwo przeoczyć**: sześć kart o literze, na którą odpowiada każde narzędzie adnotacji, o klawiszach nakładki zaznaczania i edytora w chwili, gdy to one trzymają klawiaturę, o otwieraniu pliku przez upuszczenie albo wklejenie, o powtórzeniu właśnie zrobionego zrzutu, o własnych serwerach wysyłki i o natychmiastowej powtórce. Nic nie jest narzucane: propozycja pojawia się raz po ekranie powitalnym, poza tym oprowadzanie siedzi w Ustawienia › Ogólne, samo z siebie niczego nie zmienia i można je przerwać na dowolnej karcie. Pokazywane litery narzędzi są odczytywane z samego paska narzędzi, więc nie mogą się zdezaktualizować.
