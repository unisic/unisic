...
### English
**Improved**
- **A guided tour that shows you the app instead of describing it**: seven steps that walk the window with you - each one switches to the page it is talking about and names what is on it, with the card sitting in the corner so the page stays visible and clickable underneath. It covers what each control on Capture, Record, Edit, History, Servers and Settings actually does, and finishes with the keys worth knowing: the letter every annotation tool answers to (read from the toolbar itself, so it cannot go stale) and the keys the selection overlay uses while it owns the keyboard. Two steps offer to just do the thing: take a region shot, or open the shortcut sheet. It is never forced on you - offered once after the welcome screen, otherwise in Settings › General - and it changes nothing on its own.
...
### Polski
**Ulepszone**
- **Oprowadzanie, które pokazuje aplikację, zamiast ją opisywać**: siedem kroków przechodzących razem z tobą przez okno - każdy przełącza na stronę, o której mówi, i nazywa to, co się na niej znajduje, a karta stoi w rogu, więc strona pod spodem pozostaje widoczna i klikalna. Obejmuje to, co naprawdę robi każda kontrolka na stronach Przechwytywanie, Nagrywanie, Edycja, Historia, Serwery i Ustawienia, a kończy się klawiszami wartymi zapamiętania: literą, na którą odpowiada każde narzędzie adnotacji (odczytaną z samego paska narzędzi, więc nie może się zdezaktualizować) i klawiszami nakładki zaznaczania w chwili, gdy to ona trzyma klawiaturę. Dwa kroki proponują po prostu zrobić daną rzecz: zrobić zrzut regionu albo otworzyć ściągawkę skrótów. Nic nie jest narzucane - propozycja pojawia się raz po ekranie powitalnym, poza tym oprowadzanie siedzi w Ustawienia › Ogólne - i samo z siebie niczego nie zmienia.
