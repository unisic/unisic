...
{
    // Sandboxed: the portal keys the permission on the sandbox id alone. The
    // host buckets ("" and the systemd scope) are neither ours nor writable
    // from inside, so grant exactly the one id that applies to us.
