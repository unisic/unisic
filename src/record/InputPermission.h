...
// click capture. It is deliberately NOT run at startup: the recorder calls
...
public:
    ...
    // or any failure → NoPermission. libinput is a hard build requirement, so there
    // is no third "not compiled in" answer. The context is torn down before
    // returning; no fds or threads survive the call.
    static Status probe();
